 ...
 * 企创三图信息流
 */
public class QcThreeInfoAdView extends FrameLayout {
    ...
    private InfoMoreQcAdListener mListener;
    private View                 mView;
    private int                  mWidth;
    private int                  mHeight;

    public QcThreeInfoAdView (@NonNull Context context) {
        this(context, null);
    }

    public QcThreeInfoAdView (@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        intView(context);
    }

    public QcThreeInfoAdView (@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        this(context, attrs);
    }

    public QcThreeInfoAdView (@NonNull Context context, @Nullable AttributeSet attrs, InfoMoreQcAdListener listener, NativeBean bean) {
        super(context, attrs);
        this.mContext = context;
        this.mResponse = bean;
    ...
    private void intView (final Context context) {
        mView = LayoutInflater.from(context).inflate(R.layout.item_qc_info_more, null);
        ...
        mImgPoster1 = (ImageView) mView.findViewById(R.id.img_poster1);
        mImgPoster2 = (ImageView) mView.findViewById(R.id.img_poster2);
        mImgPoster3 = (ImageView) mView.findViewById(R.id.img_poster3);
        ...
        if (assets != null && assets.size() > 0) {
            ...
            for (int i = 0; i < assets.size(); i++) {
                ...
                if (null != assetsBean.getImg()) {
                    int type = assetsBean.getImg().getType();
                    if (1 == type) {//logo图片
                        mLogoUrl = assetsBean.getImg().getUrl().get(0);
                    }

                    if (3 == type) {//广告大图
                        mImgUrl1 = assetsBean.getImg().getUrl().get(0);
                    }

                    if (4 == type) {//广告三小图
                        for (int j = 0; j < assets.size(); j++) {
                            mImgUrl1 = assetsBean.getImg().getUrl().get(0);
                            mImgUrl2 = assetsBean.getImg().getUrl().get(1);
                            mImgUrl3 = assetsBean.getImg().getUrl().get(2);
                        }
                    }
                }
                ...
                if (!TextUtils.isEmpty(mImgUrl1)) {
                    Glide.with(mContext).load(mImgUrl1).into(mImgPoster1);
                }
                if (!TextUtils.isEmpty(mImgUrl2)) {
                    Glide.with(mContext).load(mImgUrl2).into(mImgPoster2);
                }
                if (!TextUtils.isEmpty(mImgUrl3)) {
                    Glide.with(mContext).load(mImgUrl3).into(mImgPoster3);
                }

                //设置图片的大小
                LinearLayout.LayoutParams layoutParams = (LinearLayout.LayoutParams) mImgPoster1.getLayoutParams();
                int                       width        = DeviceUtil.getScreenWidth(mContext);
                layoutParams.height = (width / 3 - DeviceUtil.dip2px(mContext, 2)) * 11 / 18;
                mImgPoster1.setLayoutParams(layoutParams);

                layoutParams.setMargins(DeviceUtil.dip2px(mContext, 1), 0, 0, 0);
                mImgPoster2.setLayoutParams(layoutParams);

                RelativeLayout.LayoutParams layoutParams3 = (RelativeLayout.LayoutParams) mImgPoster3.getLayoutParams();
                layoutParams3.height = (width / 3 - DeviceUtil.dip2px(mContext, 2)) * 11 / 18;
                //layoutParams3.setMargins(DeviceUtil.dip2px(mContext, 1), 0, 0, 0);
                mImgPoster3.setLayoutParams(layoutParams3);
            ...
            //morePhotoInfomationQCADListener.morePhotoInfomationQCADFail("status:404");
